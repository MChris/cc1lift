...
class ToDo extends LongKeyedMapper[ToDo] with IdPK { 
  ...
  object priority extends MappedInt(this) {
    ...
    override def validations = validatePriority _ :: super.validations
   
    def validatePriority(in: Int) : List[FieldError] = {
      if (in > 0 && in <= 10) Nil 
      else List(FieldError(this, <b>Priority must be 1-10</b>))
    }
   
     override def _toForm = Full(select(ToDo.priorityList, Full(is.toString), f => set(f.toInt)))
  }
